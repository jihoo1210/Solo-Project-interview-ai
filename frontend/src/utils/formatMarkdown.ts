...
export function formatMarkdown(text: string): string {
  ...
  // 1. "주요 장점:" 패턴 뒤에 번호 리스트가 오는 경우
  // "...입니다. 주요 장점: 1. 항목" → "...입니다.\n\n**주요 장점:**\n\n1. 항목"
  result = result.replace(
    /([.!?])\s+([가-힣A-Za-z][가-힣A-Za-z\s]{2,30}[가-힣]):\s*(\d+)\.\s+/g,
    '$1\n\n**$2:**\n\n$3. '
  );

  // 2. 번호 리스트 내 하위 항목 패턴 (콜론이 2개인 경우)
  // "1. 가독성 향상: URI 설계: 내용" → "1. **가독성 향상:**\n   - **URI 설계:** 내용"
  result = result.replace(
    /(\d+)\.\s+([가-힣A-Za-z][가-힣A-Za-z\s\-]{1,25}[가-힣A-Za-z]):\s+([가-힣A-Za-z][가-힣A-Za-z\s\-]{1,20}[가-힣A-Za-z]):\s+/g,
    '$1. **$2:**\n   - **$3:** '
  );

  // 3. 번호 리스트 내 단일 제목 패턴 (콜론이 1개인 경우)
  // "2. 클라이언트-서버 간 명확한 계약: 일관된" → "2. **클라이언트-서버 간 명확한 계약:** 일관된"
  result = result.replace(
    /(\d+)\.\s+([가-힣A-Za-z][가-힣A-Za-z\s\-]{1,30}[가-힣A-Za-z]):\s+/g,
    '$1. **$2:** '
  );

  // 4. 번호 리스트 항목 앞에 줄바꿈 (문장 끝 뒤)
  // "...합니다. 2. 항목" → "...합니다.\n\n2. 항목"
  result = result.replace(/([.!?])\s+(\d+)\.\s+/g, '$1\n\n$2. ');

  // 5. 하위 항목 연속 패턴 (문장 끝 뒤 제목: 패턴)
  // "...합니다. HTTP 메서드 활용:" → "...합니다.\n   - **HTTP 메서드 활용:**"
  result = result.replace(
    /([.!?])\s+([가-힣A-Za-z][가-힣A-Za-z\s\-]{1,20}[가-힣A-Za-z]):\s+/g,
    '$1\n   - **$2:** '
  );

  // 6. 짧은 섹션 키워드 패턴 (장점:, 단점:, 예시:, 결론: 등)
  // "...입니다. 장점: 내용" → "...입니다.\n\n**장점:** 내용"
  const sectionKeywords = '장점|단점|예시|결론|요약|특징|방법|원인|해결|주의|참고|설명|정의|개요|목적|효과|문제|답변|질문';
  result = result.replace(
    new RegExp(`([.!?])\\s+(${sectionKeywords}):\\s+`, 'g'),
    '$1\n\n**$2:** '
  );

  // 7. 글머리 기호 리스트 앞에 줄바꿈
  // "... 내용 - 항목" → "... 내용\n- 항목"
  result = result.replace(/([가-힣A-Za-z.!?])\s+([-*+])\s+(?=[가-힣A-Za-z])/g, '$1\n$2 ');
