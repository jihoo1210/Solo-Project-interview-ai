...
export default function PaymentPage() {
  ...
  const plan = PLAN_TYPE_LABELS.PREMIUM_MONTHLY;
      ...
      <div className="w-full max-w-md sm:max-w-lg lg:max-w-xl">
        <div className="bg-white rounded-2xl shadow-lg p-5 sm:p-8 lg:p-10">
          ...
          <p className="text-center text-text-muted mb-6 sm:mb-8 text-sm sm:text-base">
            매달 자동 결제로 편리하게 이용하세요
          </p>

          {/* 플랜 정보 */}
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 sm:p-6 lg:p-8 mb-6 border border-amber-200">
            ...
            <div className="flex items-baseline gap-1 mb-4 sm:mb-6">
              <span className="text-2xl sm:text-3xl lg:text-4xl font-bold text-text">{plan.price.toLocaleString()}</span>
              <span className="text-text-muted text-sm sm:text-base">원</span>
              <span className="text-text-muted text-xs sm:text-sm">/ {plan.duration}</span>
            </div>
          ...
          <div className="mb-6 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs sm:text-sm text-blue-700">
              <strong>정기결제 안내:</strong> 카드를 등록하면 매월 자동으로 결제됩니다.
          ...
          <div className="space-y-3">
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span>월 {plan.price.toLocaleString()}원 정기구독</span>
                </>
              )}
            </button>
